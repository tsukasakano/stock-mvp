...
import { runWalkForward, type WalkForwardResult } from '@/lib/walkForward';
  ...
  return (
    <div className="space-y-1.5">
      <div className="flex items-center justify-between text-xs">
        <span className="text-slate-400">一貫性スコア</span>
        <span className="font-mono font-bold" style={{ color }}>{value.toFixed(3)}</span>
      </div>
...
export default function WalkForward() {
  ...
  const [trainPeriod, setTrainPeriod] = useState('252');
  const [testPeriod, setTestPeriod] = useState('63');
    ...
    try {
      ...
      if (!chartData) {
        if (useHistorical) {
          ...
          if (!raw) {
            setLoadError('リアルデータが見つかりません。fetch_historical.py を実行してください。');
            return;
          }
      ...
      const result = runWalkForward({
        ...
        trainPeriod: parseInt(trainPeriod) || 252,
        testPeriod:  parseInt(testPeriod)  || 63,
  ...
  }, [rules, buyRuleId, sellRuleId, selectedStockValue, selectedStock, useHistorical, trainPeriod, testPeriod, takeProfit, trailingStop, maxHoldDays, commissionRate, slippage]);

  const chartData = wfResult?.windows.map((w, i) => ({
    window: `W${i + 1}`,
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div className="grid grid-cols-2 gap-2">
            <div>
              ...
              <input type="number" value={trainPeriod} min={60} step={21}
                onChange={e => { setTrainPeriod(e.target.value); setWfResult(null); }}
                className={inputCls} />
            </div>
            <div>
              <label className="text-xs text-slate-500 block mb-1">検証期間（日）</label>
              <input type="number" value={testPeriod} min={21} step={21}
                onChange={e => { setTestPeriod(e.target.value); setWfResult(null); }}
                className={inputCls} />
            </div>
          </div>
        </div>
          ...
          {wfResult.windows.length === 0 ? (
            ...
            <>
              ...
              <div className="bg-slate-900/60 rounded-xl p-4 border border-slate-800/60">
                <ConsistencyGauge value={wfResult.consistency} />
                <p className="text-[10px] text-slate-600 mt-2">
                  一貫性スコアは学習期間と検証期間のリターンのPearson相関係数です。0.7以上で過学習なし、0.4〜0.7は要注意、0.4未満は過学習の可能性があります。
                </p>
              </div>
              ...
              {chartData.length > 0 && (
                <div className="bg-slate-900 rounded-xl p-4 border border-slate-800">
                  <p className="text-xs font-semibold text-slate-400 mb-3">ウィンドウ別リターン（学習 vs 検証）</p>
                  <ResponsiveContainer width="100%" height={220}>
                    <LineChart data={chartData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                      <CartesianGrid strokeDasharray="3 3" stroke="#1e293b" />
                      <XAxis dataKey="window" tick={{ fontSize: 10, fill: '#64748b' }} />
                      <YAxis tick={{ fontSize: 10, fill: '#64748b' }} tickFormatter={v => `${v}%`} width={40} />
                      <Tooltip
                        contentStyle={TOOLTIP_STYLE}
                        formatter={(v: unknown, name: unknown) => {
                          const num = Number(v);
                          return [`${num >= 0 ? '+' : ''}${num}%`, name === 'train' ? '学習期間' : '検証期間'];
                        }}
                        labelFormatter={(_, payload) => {
                          const p = payload?.[0]?.payload;
                          return p ? `W${payload[0]?.payload?.window?.replace('W','') ?? ''} 検証: ${p.testLabel}` : '';
                        }}
                      />
                      <Legend formatter={(v) => v === 'train' ? '学習期間' : '検証期間'} />
                      <ReferenceLine y={0} stroke="#475569" strokeDasharray="4 2" />
                      <Line type="monotone" dataKey="train" stroke="#3b82f6" strokeWidth={2} dot={{ r: 3 }} />
                      <Line type="monotone" dataKey="test"  stroke="#10b981" strokeWidth={2} dot={{ r: 3 }} strokeDasharray="5 3" />
                    </LineChart>
                  </ResponsiveContainer>
                </div>
              )}
                  ...
                  <table className="w-full text-xs">
                    <thead>
                      <tr className="border-b border-slate-800 text-slate-500">
                        ...
                        <th className="px-3 py-2 text-right font-medium">学習リターン</th>
                        <th className="px-3 py-2 text-right font-medium">検証リターン</th>
                        <th className="px-3 py-2 text-right font-medium">検証勝率</th>
                        <th className="px-3 py-2 text-right font-medium">検証シャープ</th>
                      </tr>
                    </thead>
                    <tbody>
                      {wfResult.windows.map((w, i) => {
                        ...
                        return (
                          <tr key={i} className="border-b border-slate-800/60 hover:bg-slate-800/30 transition-colors">
                            <td className="px-3 py-2.5 font-mono text-slate-500">W{i + 1}</td>
